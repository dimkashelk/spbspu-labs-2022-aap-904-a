#ifndef MATRIXOPERATIONS_H
#define MATRIXOPERATIONS_H
#include <cstddef>
#include "matrix.h"
namespace turkin
{
  size_t getNotZeroLines(const int * matrix, size_t mx, size_t my);
  size_t getSameSumLines(const int * matrix, size_t mx, size_t my);
  size_t getSaddlePoints(const turkin::Matrix & matrix);
  size_t getSumOfModules(const turkin::Matrix & matrix);
  int getSumOfLine(const int * matrix, size_t mx, size_t my, size_t line);
  turkin::Matrix getSmoothedMatrix(const turkin::Matrix & matrix);
}
#endif
